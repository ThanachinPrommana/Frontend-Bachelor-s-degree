...
import { Trash2, Image as ImageIcon, Video as VideoIcon, Info } from "lucide-react";
...
          setError(`ไฟล์รูปภาพ "${file.name}" ขนาดเกิน ${prettySizeMB(MAX_IMAGE_SIZE)}`);
          return false;
        }
      }
...
          setError(`ไฟล์วิดีโอ "${file.name}" ขนาดเกิน ${prettySizeMB(MAX_VIDEO_SIZE)}`);
          return false;
        }
      }
...
      setError("");
      form.setValue("videos", updated, { shouldValidate: true, shouldDirty: true });
      return true;
    },
    [form, videos]
  );
...
  };
  const handleDragOverVideo = (e) => { e.preventDefault(); setIsDraggingVideo(true); };
  const handleDragLeaveVideo = () => setIsDraggingVideo(false);
...
    (allData.videos || []).forEach((vid) => formData.append("videos", vid.file));
...
                <h3 className="font-semibold">รูปภาพ (สูงสุด {MAX_IMAGES} รูป, ไฟล์ละไม่เกิน {prettySizeMB(MAX_IMAGE_SIZE)})</h3>
              </div>
              {/* Dropzone for images */}
              <div
                role="button"
...
                  canAddMoreImages ? "cursor-pointer hover:bg-primary/10" : "opacity-60 cursor-not-allowed"
                } ${isDragging ? "border-primary bg-primary/10" : "border-primary/40"}`}
              >
                <p className="font-medium">
                  {canAddMoreImages ? "ลากและวางรูปภาพที่นี่ หรือคลิกเพื่อเลือกไฟล์" : "ครบจำนวนรูปแล้ว"}
                </p>
                <p className="text-xs text-muted-foreground mt-1">อัปโหลดแล้ว {images.length} / {MAX_IMAGES} รูป</p>
                <input
                  ref={fileInputRef}
                  type="file"
                  accept="image/*"
                  multiple
                  onChange={(e) => { if (handleAddImages(e.target.files)) e.target.value = ""; }}
                  className="hidden"
                />
              </div>
...
                  {images.map((img, idx) => (
                    <div key={idx} className="relative group rounded-lg overflow-hidden ring-1 ring-black/5">
                      <img src={img.preview} alt={img.name || `preview-${idx}`} className="w-full h-36 object-cover" />
                      <div className="absolute left-2 top-2 text-xs px-2 py-0.5 rounded-full bg-black/60 text-white">{idx + 1}</div>
                      <button type="button" onClick={() => handleRemoveImage(idx)} className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1.5 shadow-md opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`ลบรูปที่ ${idx + 1}`}>
                        <Trash2 size={16} />
                      </button>
                      <div className="px-2 py-1">
                        <p className="text-xs truncate">{img.name || "image"}</p>
                        <p className="text-[10px] text-muted-foreground">{prettySizeMB(img.size)}</p>
                      </div>
                    </div>
                  ))}
                </div>
...
                <h3 className="font-semibold">วิดีโอ (สูงสุด {MAX_VIDEOS} ไฟล์, ไฟล์ละไม่เกิน {prettySizeMB(MAX_VIDEO_SIZE)})</h3>
              </div>
              {/* Dropzone for videos */}
              <div
                role="button"
...
                onKeyDown={(e) => e.key === "Enter" || e.key === " " ? openVideoPicker() : null}
                onDrop={handleDropVideos}
                onDragOver={handleDragOverVideo}
                onDragLeave={handleDragLeaveVideo}
                className={`rounded-xl border-2 border-dashed transition-colors p-6 md:p-8 text-center ${
                  canAddMoreVideos ? "cursor-pointer hover:bg-primary/10" : "opacity-60 cursor-not-allowed"
                } ${isDraggingVideo ? "border-primary bg-primary/10" : "border-primary/40"}`}
              >
                <p className="font-medium">
                  {canAddMoreVideos ? "ลากและวางวิดีโอที่นี่ หรือคลิกเพื่อเลือกไฟล์" : "ครบจำนวนวิดีโอแล้ว"}
                </p>
                <p className="text-xs text-muted-foreground mt-1">อัปโหลดแล้ว {videos.length} / {MAX_VIDEOS} วิดีโอ</p>
                <input
                  ref={videoInputRef}
                  type="file"
                  accept="video/*"
                  multiple
                  onChange={(e) => { if (handleAddVideos(e.target.files)) e.target.value = ""; }}
                  className="hidden"
                />
              </div>
...
                  {videos.map((vid, idx) => (
                    <div key={idx} className="relative group rounded-lg overflow-hidden ring-1 ring-black/5">
                      <video src={vid.preview} poster={vid.thumbnail} className="w-full h-36 object-cover bg-black" />
                      <div className="absolute left-2 top-2 text-xs px-2 py-0.5 rounded-full bg-black/60 text-white">{idx + 1}</div>
                      <button type="button" onClick={() => handleRemoveVideo(idx)} className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1.5 shadow-md opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`ลบวิดีโอที่ ${idx + 1}`}>
                        <Trash2 size={16} />
                      </button>
                      <div className="px-2 py-1">
                        <p className="text-xs truncate">{vid.name}</p>
                        <p className="text-[10px] text-muted-foreground">{prettySizeMB(vid.size)}</p>
                      </div>
                    </div>
                  ))}
                </div>
...
              <Button variant="outline" onClick={() => navigate("/seller/post-for-sale/inform")}>
                ย้อนกลับ
              </Button>
